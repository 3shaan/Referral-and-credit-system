  ...
  return (
    <Activity mode={isOpen ? 'visible' : 'hidden'}>
      ...
      <div className="fixed inset-0 z-40 lg:hidden">
        <div role="button" tabIndex={0} className="absolute inset-0 bg-gray-600 opacity-75" onKeyDown={() => toggle()}></div>
        <aside className="absolute inset-y-0 left-0 flex flex-col w-64 bg-white">
          <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200">
            <h1 className="text-xl font-bold text-gray-900">Dashboard</h1>
            <button type="button" onClick={() => toggle()} className="text-gray-500 hover:text-gray-700">
              <X className="w-6 h-6" />
            </button>
          </div>
